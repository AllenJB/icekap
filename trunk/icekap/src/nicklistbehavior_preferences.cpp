...
void NicklistBehavior_Config::setNickList(const QString &sortingOrder)
{
  ...
  for(unsigned int index=sortingOrder.length();index!=0;index--)
  {
    ...
    if(mode=='v') new KListViewItem(sortOrder,mode,i18n("Voice"));
    if(mode=='h') new KListViewItem(sortOrder,mode,i18n("Halfops"));
    if(mode=='o') new KListViewItem(sortOrder,mode,i18n("Operators"));
    if(mode=='p') new KListViewItem(sortOrder,mode,i18n("Channel Admins"));
    if(mode=='q') new KListViewItem(sortOrder,mode,i18n("Channel Owners"));
  }
}
