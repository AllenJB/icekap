...
void SSLSocket::connected()
{
    ...
    else
    {
        ...
        emit sslFailure(i18n("The functionality to connect to servers using encrypted SSL communications is not available to Icekap because OpenSSL support was not enabled at compile time.  You will need to get new version of KDE that has SSL support."));
        close();
    }
}
