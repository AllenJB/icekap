...
namespace Konversation
{
        ...
        : KDialogBase(Swallow, title, Ok|Cancel, Ok, parent, name)
    {
        ...
        QWhatsThis::add(m_mainWidget->m_commandEdit, i18n("Optional. This command will be sent to the server after connecting. Example: <b>/msg NickServ IDENTIFY <i>icekaprocks</i></b>. This example is for the freenode network, which requires users to register their nickname with a password and login when connecting. <i>icekaprocks<i> is the password for the nickname given in Identity. You may enter more than one command by separating them with semicolons."));
        m_mainWidget->m_commandsLabel->setBuddy(m_mainWidget->m_commandEdit);

        QWhatsThis::add(m_mainWidget->m_autoConnectCBox, i18n("Check here if you want to automatically connect to this network whenever you open Icekap."));

        QWhatsThis::add(m_mainWidget->m_serverLBox, i18n("This is a list of IRC Servers in the network. When connecting to the network, Icekap will attempt to connect to the top server first. If this fails, it will attempt the second server. If this fails, it will attempt the third, and so on. At least one server must be specified. Click a server to highlight it."));
        ...
        QWhatsThis::add(m_mainWidget->m_channelLBox, i18n("Optional. This is a list of the channels that will be automatically joined once Icekap has connected to a server. You may leave this blank if you wish to not automatically join any channels."));
